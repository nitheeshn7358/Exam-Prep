  ...
  useEffect(() => {
    api.get(`/questions/${noteId}`)
      .then((res) => {
        ...
        setLoading(false)
        startSession(parseInt(noteId), '', 'quiz', shuffled.length)
      })
      .catch(() => {
        setError('Failed to load questions.')
        setLoading(false)
      })
    return () => {} // keep session alive when navigating away
  }, [noteId])
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      ...
      <nav className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-6 py-4 flex items-center justify-between">
        ...
        <Link to="/dashboard" className="text-sm text-gray-500 hover:text-indigo-600 transition flex items-center gap-1">
